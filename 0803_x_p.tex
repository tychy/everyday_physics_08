...
\date{\today}
\author{山田龍}
\title{大学ではなぜ速度でなく運動量が使われるか}
\begin{document}
\maketitle
利点をいくつか書いてみます。
ラグランジュ形式では,$\dot{q}$が$q$によっていたので、そもそもオイラーラグランジュ方程式が二階微分を含む。
ラグランジュ形式とハミルトン形式はルジャンドル変換でつながっているので、基本的には等価だと思う。
ラグランジュ形式も座標によらなかったがそれは同様。（理論がきれい？）
\end{document}
